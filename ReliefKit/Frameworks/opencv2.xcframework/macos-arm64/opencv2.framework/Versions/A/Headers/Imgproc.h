 ...
 *
 * - SeeAlso ``sepFilter2D:dst:ddepth:kernelX:kernelY:anchor:delta:borderType:``, ``getDerivKernels:ky:dx:dy:ksize:normalize:ktype:``, ``getStructuringElement:ksize:anchor:``, ``GaussianBlur:dst:ksize:sigmaX:sigmaY:borderType:hint:``
 */
 ...
 * `ksize=FILTER_SCHARR`, the Scharr `$$3 \times 3$$` kernels are generated (see ``Imgproc/Scharr``). Otherwise, Sobel
 * kernels are generated (see ``Imgproc/Sobel``). The filters are normally passed to ``Imgproc/sepFilter2D`` or to
 *
 ...
 *
 * The function constructs and returns the structuring element that can be further passed to ``Imgproc/erode``,
 * ``Imgproc/dilate`` or ``Imgproc/morphologyEx``. But you can also construct an arbitrary binary mask yourself and use it as
 ...
 *
 * Note:* The median filter uses ``BorderTypes/BORDER_REPLICATE`` internally to cope with border pixels, see ``BorderTypes``
 *
 ...
 *
 * - SeeAlso ``bilateralFilter:dst:d:sigmaColor:sigmaSpace:borderType:``, ``blur:dst:ksize:anchor:borderType:``, ``boxFilter:dst:ddepth:ksize:anchor:normalize:borderType:``, ``GaussianBlur:dst:ksize:sigmaX:sigmaY:borderType:hint:``
 */
 ...
 * respectively (see ``Imgproc/getGaussianKernel`` for details); to fully control the result regardless of
 * possible future modifications of all this semantics, it is recommended to specify all of ksize,
 * sigmaX, and sigmaY.
 * @param borderType pixel extrapolation method, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 * @param hint Implementation modfication flags. See ``AlgorithmHint``
 *
 * - SeeAlso ``sepFilter2D:dst:ddepth:kernelX:kernelY:anchor:delta:borderType:``, ``filter2D:dst:ddepth:kernel:anchor:delta:borderType:``, ``blur:dst:ksize:anchor:borderType:``, ``boxFilter:dst:ddepth:ksize:anchor:normalize:borderType:``, ``bilateralFilter:dst:d:sigmaColor:sigmaSpace:borderType:``, ``medianBlur:dst:ksize:``
 */
 ...
 * algorithms, and so on). If you need to compute pixel sums over variable-size windows, use ``Imgproc/integral``.
 *
 ...
 * @param borderType border mode used to extrapolate pixels outside of the image, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``blur:dst:ksize:anchor:borderType:``, ``bilateralFilter:dst:d:sigmaColor:sigmaSpace:borderType:``, ``GaussianBlur:dst:ksize:sigmaX:sigmaY:borderType:hint:``, ``medianBlur:dst:ksize:``, ``integral:sum:sdepth:``
 */
 ...
 * @param borderType border mode used to extrapolate pixels outside of the image, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``boxFilter:dst:ddepth:ksize:anchor:normalize:borderType:``
 */
 ...
 * @param borderType border mode used to extrapolate pixels outside of the image, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``boxFilter:dst:ddepth:ksize:anchor:normalize:borderType:``, ``bilateralFilter:dst:d:sigmaColor:sigmaSpace:borderType:``, ``GaussianBlur:dst:ksize:sigmaX:sigmaY:borderType:hint:``, ``medianBlur:dst:ksize:``
 */
 ...
 * the kernel using ``Core/flip`` and set the new anchor to `(kernel.cols - anchor.x - 1, kernel.rows -
 * anchor.y - 1)`.
 *
 ...
 * @param borderType pixel extrapolation method, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``sepFilter2D:dst:ddepth:kernelX:kernelY:anchor:delta:borderType:``, dft, ``matchTemplate:templ:result:method:mask:``
 */
 ...
 * @param borderType Pixel extrapolation method, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``filter2D:dst:ddepth:kernel:anchor:delta:borderType:``, ``Sobel:dst:ddepth:dx:dy:ksize:scale:delta:borderType:``, ``GaussianBlur:dst:ksize:sigmaX:sigmaY:borderType:hint:``, ``boxFilter:dst:ddepth:ksize:anchor:normalize:borderType:``, ``blur:dst:ksize:anchor:borderType:``
 */
 ...
 *
 * There is also the special value `ksize = ``SpecialFilter/FILTER_SCHARR`` (-1)` that corresponds to the `$$3\times3$$` Scharr
 ...
 * applied (see ``Imgproc/getDerivKernels`` for details).
 * @param delta optional delta value that is added to the results prior to storing them in dst.
 * @param borderType pixel extrapolation method, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``Scharr:dst:ddepth:dx:dy:scale:delta:borderType:``, ``Laplacian:dst:ddepth:ksize:scale:delta:borderType:``, ``sepFilter2D:dst:ddepth:kernelX:kernelY:anchor:delta:borderType:``, ``filter2D:dst:ddepth:kernel:anchor:delta:borderType:``, ``GaussianBlur:dst:ksize:sigmaX:sigmaY:borderType:hint:``, cartToPolar
 */
 ...
 *                   Only #BORDER_DEFAULT=#BORDER_REFLECT_101 and ``BorderTypes/BORDER_REPLICATE`` are supported.
 *
 * - SeeAlso ``Sobel:dst:ddepth:dx:dy:ksize:scale:delta:borderType:``
 */
 ...
 * @param dst Destination image of the same size and the same number of channels as src .
 * @param ddepth Desired depth of the destination image, see *Ref:* filter_depths "combinations".
 * @param ksize Aperture size used to compute the second-derivative filters. See ``Imgproc/getDerivKernels`` for
 ...
 * @param borderType Pixel extrapolation method, see ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``Sobel:dst:ddepth:dx:dy:ksize:scale:delta:borderType:``, ``Scharr:dst:ddepth:dx:dy:scale:delta:borderType:``
 */
 ...
 * src .
 * @param blockSize Neighborhood size (see the details on ``Imgproc/cornerEigenValsAndVecs`` ).
 ...
 * @param borderType Pixel extrapolation method. See ``BorderTypes``. ``BorderTypes/BORDER_WRAP`` is not supported.
 *
 * - SeeAlso ``cornerMinEigenVal:dst:blockSize:ksize:borderType:``, ``cornerHarris:dst:blockSize:ksize:k:borderType:``, ``preCornerDetect:dst:ksize:borderType:``
 */
 ...
 *     ``Imgproc/cornerMinEigenVal`` or ``Imgproc/cornerHarris`` .
 ...
 * (see ``Imgproc/cornerMinEigenVal`` ) or the Harris function response (see ``Imgproc/cornerHarris`` ). The corners with the
 ...
 * @param useHarrisDetector Parameter indicating whether to use a Harris detector (see ``Imgproc/cornerHarris``)
 * or ``Imgproc/cornerMinEigenVal``.
 * @param k Free parameter of the Harris detector.
 *
 * - SeeAlso ``cornerMinEigenVal:dst:blockSize:ksize:borderType:``, ``cornerHarris:dst:blockSize:ksize:k:borderType:``, calcOpticalFlowPyrLK, estimateRigidTransform, 
 */
 ...
 * you know it. Or, in the case of ``HoughModes/HOUGH_GRADIENT`` method you may set maxRadius to a negative number
 ...
 * @param method Detection method, see ``HoughModes``. The available methods are ``HoughModes/HOUGH_GRADIENT`` and ``HoughModes/HOUGH_GRADIENT_ALT``.
 * @param dp Inverse ratio of the accumulator resolution to the image resolution. For example, if
 * dp=1 , the accumulator has the same resolution as the input image. If dp=2 , the accumulator has
 * half as big width and height. For ``HoughModes/HOUGH_GRADIENT_ALT`` the recommended value is dp=1.5,
 * unless some small very circles need to be detected.
 * @param minDist Minimum distance between the centers of the detected circles. If the parameter is
 * too small, multiple neighbor circles may be falsely detected in addition to a true one. If it is
 * too large, some circles may be missed.
 * @param param1 First method-specific parameter. In case of ``HoughModes/HOUGH_GRADIENT`` and ``HoughModes/HOUGH_GRADIENT_ALT``,
 * it is the higher threshold of the two passed to the Canny edge detector (the lower one is twice smaller).
 * Note that ``HoughModes/HOUGH_GRADIENT_ALT`` uses ``Imgproc/Scharr`` algorithm to compute image derivatives, so the threshold value
 * should normally be higher, such as 300 or normally exposed and contrasty images.
 * @param param2 Second method-specific parameter. In case of ``HoughModes/HOUGH_GRADIENT``, it is the
 * accumulator threshold for the circle centers at the detection stage. The smaller it is, the more
 * false circles may be detected. Circles, corresponding to the larger accumulator values, will be
 * returned first. In the case of ``HoughModes/HOUGH_GRADIENT_ALT`` algorithm, this is the circle "perfectness" measure.
 * The closer it to 1, the better shaped circles algorithm selects. In most cases 0.9 should be fine.
 * If you want get better detection of small circles, you may decrease it to 0.85, 0.8 or even less.
 * But then also try to limit the search range [minRadius, maxRadius] to avoid many false circles.
 * @param minRadius Minimum circle radius.
 * @param maxRadius Maximum circle radius. If <= 0, uses the maximum image dimension. If < 0, ``HoughModes/HOUGH_GRADIENT`` returns
 * centers without finding the radius. ``HoughModes/HOUGH_GRADIENT_ALT`` always computes circle radiuses.
 *
 * - SeeAlso ``fitEllipse:``, ``minEnclosingCircle:center:radius:``
 */
 ...
 * structuring element is used. Kernel can be created using ``Imgproc/getStructuringElement``.
 ...
 *
 * - SeeAlso ``dilate:dst:kernel:anchor:iterations:borderType:borderValue:``, ``morphologyEx:dst:op:kernel:anchor:iterations:borderType:borderValue:``, ``getStructuringElement:ksize:anchor:``
 */
 ...
 * @param borderValue border value in case of a constant border
 *
 * - SeeAlso ``erode:dst:kernel:anchor:iterations:borderType:borderValue:``, ``morphologyEx:dst:op:kernel:anchor:iterations:borderType:borderValue:``, ``getStructuringElement:ksize:anchor:``
 */
 ...
 * @param kernel Structuring element. It can be created using ``Imgproc/getStructuringElement``.
 ...
 * meaning.
 * - SeeAlso ``dilate:dst:kernel:anchor:iterations:borderType:borderValue:``, ``erode:dst:kernel:anchor:iterations:borderType:borderValue:``, ``getStructuringElement:ksize:anchor:``
 * Note:* The number of iterations is the number of times erosion or dilatation operation will be applied.
 * For instance, an opening operation (``MorphTypes/MORPH_OPEN``) with two iterations is equivalent to apply
 ...
 *
 * To shrink an image, it will generally look best with ``InterpolationFlags/INTER_AREA`` interpolation, whereas to
 * enlarge an image, it will generally look best with ``InterpolationFlags/INTER_CUBIC`` (slow) or ``InterpolationFlags/INTER_LINEAR``
 ...
 *
 * - SeeAlso ``warpAffine:dst:M:dsize:flags:borderMode:borderValue:``, ``warpPerspective:dst:M:dsize:flags:borderMode:borderValue:``, ``remap:dst:map1:map2:interpolation:borderMode:borderValue:``
 */
 ...
 *
 * when the flag ``InterpolationFlags/WARP_INVERSE_MAP`` is set. Otherwise, the transformation is first inverted
 * with ``Imgproc/invertAffineTransform`` and then put in the formula above instead of M. The function cannot
 * operate in-place.
 *
 * @param src input image.
 ...
 * flag ``InterpolationFlags/WARP_INVERSE_MAP`` that means that M is the inverse transformation (
 ...
 *
 * - SeeAlso ``warpPerspective:dst:M:dsize:flags:borderMode:borderValue:``, ``resize:dst:dsize:fx:fy:interpolation:``, ``remap:dst:map1:map2:interpolation:borderMode:borderValue:``, ``getRectSubPix:patchSize:center:patch:patchType:``, transform
 */
 ...
 *
 * when the flag ``InterpolationFlags/WARP_INVERSE_MAP`` is set. Otherwise, the transformation is first inverted with invert
 ...
 * @param dsize size of the output image.
 * @param flags combination of interpolation methods (``InterpolationFlags/INTER_LINEAR`` or ``InterpolationFlags/INTER_NEAREST``) and the
 * optional flag ``InterpolationFlags/WARP_INVERSE_MAP``, that sets M as the inverse transformation (
 * `$$\texttt{dst}\rightarrow\texttt{src}$$` ).
 * @param borderMode pixel extrapolation method (``BorderTypes/BORDER_CONSTANT`` or ``BorderTypes/BORDER_REPLICATE``).
 * @param borderValue value used in case of a constant border; by default, it equals 0.
 *
 * - SeeAlso ``warpAffine:dst:M:dsize:flags:borderMode:borderValue:``, ``resize:dst:dsize:fx:fy:interpolation:``, ``remap:dst:map1:map2:interpolation:borderMode:borderValue:``, ``getRectSubPix:patchSize:center:patch:patchType:``, perspectiveTransform
 */
 ...
 * `$$map_1$$`, or fixed-point maps created by using ``Imgproc/convertMaps``. Fixed-point maps
 ...
 * `$$map_1$$` contains pairs (cvFloor(x),
 ...
 * CV_32FC1, or CV_32FC2. See ``Imgproc/convertMaps`` for details on converting a floating point
 * representation to fixed-point.
 * @param map2 The second map of y values having the type CV_16UC1, CV_32FC1, or none (empty map
 * if map1 is (x,y) points), respectively.
 * @param interpolation Interpolation method (see ``InterpolationFlags``). The methods ``InterpolationFlags/INTER_AREA``
 * ``InterpolationFlags/INTER_LINEAR_EXACT`` and ``InterpolationFlags/INTER_NEAREST_EXACT`` are not supported by this function.
 ...
 * most frequently used conversion operation, in which the original floating-point maps (see ``Imgproc/remap``)
 ...
 *
 * - SeeAlso ``remap:dst:map1:map2:interpolation:borderMode:borderValue:``, undistort, initUndistortRectifyMap
 */
 ...
 *
 * - SeeAlso ``getAffineTransform:dst:``, ``warpAffine:dst:M:dsize:flags:borderMode:borderValue:``, transform
 */
 ...
 *
 * - SeeAlso findHomography, ``warpPerspective:dst:M:dsize:flags:borderMode:borderValue:``, perspectiveTransform
 */
 ...
 *
 * - SeeAlso ``warpAffine:dst:M:dsize:flags:borderMode:borderValue:``, ``warpPerspective:dst:M:dsize:flags:borderMode:borderValue:``
 */
 ...
 * -   To calculate magnitude and angle in degrees ``Core/cartToPolar`` is used internally thus angles are measured from 0 to 360 with accuracy about 0.3 degrees.
 *
 * - SeeAlso cv::linearPolar
 */
 ...
 * ![Polar remaps reference](polar_remap_doc)
 *
 ...
 * \par Reverse mapping
 *
 * You can get reverse mapping adding ``InterpolationFlags/WARP_INVERSE_MAP`` to `flags`
 ...
 *             - Add ``WarpPolarMode/WARP_POLAR_LINEAR`` to select linear polar mapping (default)
 *             - Add ``WarpPolarMode/WARP_POLAR_LOG`` to select semilog polar mapping
 *             - Add ``InterpolationFlags/WARP_INVERSE_MAP`` for reverse mapping.
 * Note:*
 * -  The function can not operate in-place.
 * -  To calculate magnitude and angle in degrees ``Core/cartToPolar`` is used internally thus angles are measured from 0 to 360 with accuracy about 0.3 degrees.
 * -  This function uses ``Imgproc/remap``. Due to current implementation limitations the size of an input and output images should be less than 32767x32767.
 *
 * - SeeAlso cv::remap
 */
 ...
 *
 * ![integral calculation example](integral)
 *
 ...
 *
 * - SeeAlso ``accumulateSquare:dst:mask:``, ``accumulateProduct:src2:dst:mask:``, ``accumulateWeighted:dst:alpha:mask:``
 */
 ...
 * @param mask Optional operation mask.
 *
 * - SeeAlso ``accumulate:dst:mask:``, ``accumulateSquare:dst:mask:``, ``accumulateWeighted:dst:alpha:mask:``
 */
 ...
 * @param mask Optional operation mask.
 *
 * - SeeAlso ``accumulate:dst:mask:``, ``accumulateSquare:dst:mask:``, ``accumulateProduct:src2:dst:mask:``
 */
 ...
 *
 * - SeeAlso dft, getOptimalDFTSize, idft, mulSpectrums createHanningWindow
 */
 ...
 *
 * - SeeAlso ``phaseCorrelate:src2:window:response:``, dft, idft, ``createHanningWindow:winSize:type:``
 */
+ (Point2d*)phaseCorrelateIterative:(Mat*)src1 src2:(Mat*)src2 L2size:(int)L2size maxIters:(int)maxIters NS_SWIFT_NAME(phaseCorrelateIterative(src1:src2:L2size:maxIters:));
 ...
 * - SeeAlso ``phaseCorrelate:src2:window:response:``, dft, idft, ``createHanningWindow:winSize:type:``
 */
+ (Point2d*)phaseCorrelateIterative:(Mat*)src1 src2:(Mat*)src2 L2size:(int)L2size NS_SWIFT_NAME(phaseCorrelateIterative(src1:src2:L2size:));
 ...
 * - SeeAlso ``phaseCorrelate:src2:window:response:``, dft, idft, ``createHanningWindow:winSize:type:``
 */
+ (Point2d*)phaseCorrelateIterative:(Mat*)src1 src2:(Mat*)src2 NS_SWIFT_NAME(phaseCorrelateIterative(src1:src2:));
 ...
 * used to get a bi-level (binary) image out of a grayscale image ( ``Core/compare`` could be also used for
 * this purpose) or for removing a noise, that is, filtering out pixels with too small or too large
 * values. There are several types of thresholding supported by the function. They are determined by
 * type parameter.
 *
 * Also, the special values ``ThresholdTypes/THRESH_OTSU`` or ``ThresholdTypes/THRESH_TRIANGLE`` may be combined with one of the
 ...
 * @param maxval maximum value to use with the ``ThresholdTypes/THRESH_BINARY`` and ``ThresholdTypes/THRESH_BINARY_INV`` thresholding
 * types.
 * @param type thresholding type (see ``ThresholdTypes``).
 * @return the computed threshold value if Otsu's or Triangle methods used.
 *
 * - SeeAlso ``thresholdWithMask:dst:mask:thresh:maxval:type:``, ``adaptiveThreshold:dst:maxValue:adaptiveMethod:thresholdType:blockSize:C:``, ``findContours:contours:hierarchy:mode:method:offset:``, compare, min, max
 */
...
//
/**
 * Same as ``Imgproc/threshold``, but with an optional mask
 *
 * Note:* If the mask is empty, *thresholdWithMask* is equivalent to ``Imgproc/threshold``.
 ...
 * @return the computed threshold value if Otsu's or Triangle methods used.
 *
 * - SeeAlso ``threshold:dst:thresh:maxval:type:``, ``adaptiveThreshold:dst:maxValue:adaptiveMethod:thresholdType:blockSize:C:``, ``findContours:contours:hierarchy:mode:method:offset:``, compare, min, max
 */
 ...
 * The ``BorderTypes/BORDER_REPLICATE`` | ``BorderTypes/BORDER_ISOLATED`` is used to process boundaries.
 * @param thresholdType Thresholding type that must be either ``ThresholdTypes/THRESH_BINARY`` or ``ThresholdTypes/THRESH_BINARY_INV``,
 * see ``ThresholdTypes``.
 ...
 *
 * - SeeAlso ``threshold:dst:thresh:maxval:type:``, ``blur:dst:ksize:anchor:borderType:``, ``GaussianBlur:dst:ksize:sigmaX:sigmaY:borderType:hint:``
 */
 ...
 * @param borderType Pixel extrapolation method, see ``BorderTypes`` (``BorderTypes/BORDER_CONSTANT`` isn't supported)
 */
 ...
 * @param dst output image. It has the specified size and the same type as src .
 * @param dstsize size of the output image.
 * @param borderType Pixel extrapolation method, see ``BorderTypes`` (only ``BorderTypes/BORDER_DEFAULT`` is supported)
 */
 ...
 * or more general sparse configurations of weighted points, consider using the ``Imgproc/EMD`` function.
 *
 ...
 * mask using ``Imgproc/findContours`` and ``Imgproc/drawContours`` (see the watershed.cpp demo). The markers are "seeds" of
 ...
 * size as image .
 *
 * - SeeAlso ``findContours:contours:hierarchy:mode:method:offset:``
 */
 ...
 * mode is set to ``GrabCutModes/GC_INIT_WITH_RECT``. Its elements may have one of the ``GrabCutClasses``.
 ...
 *
 * When maskSize == ``DistanceTransformMasks/DIST_MASK_PRECISE`` and distanceType == ``DistanceTypes/DIST_L2`` , the function runs the
 ...
 * same cost (denoted as `c`). For the ``DistanceTypes/DIST_C`` and ``DistanceTypes/DIST_L1`` types, the distance is calculated
 * precisely, whereas for ``DistanceTypes/DIST_L2`` (Euclidean distance) the distance can be calculated only with a
 ...
 * - DIST_C: `a = 1, b = 1`
 *
 * Typically, for a fast, coarse distance estimation ``DistanceTypes/DIST_L2``, a `$$3\times 3$$` mask is used. For a
 * more accurate distance estimation ``DistanceTypes/DIST_L2``, a `$$5\times 5$$` mask or the precise algorithm is used.
 ...
 * ``DistanceTransformMasks/DIST_MASK_PRECISE`` is not supported by this variant. In case of the ``DistanceTypes/DIST_L1`` or ``DistanceTypes/DIST_C`` distance type,
 ...
 * ``DistanceTypes/DIST_L1`` or ``DistanceTypes/DIST_C`` distance type, the parameter is forced to 3 because a `$$3\times 3$$` mask gives
 * the same result as `$$5\times 5$$` or any larger aperture.
 * @param dstType Type of output image. It can be CV_8U or CV_32F. Type CV_8U can be used only for
 * the first variant of the function and distanceType == ``DistanceTypes/DIST_L1``.
 */
 ...
 * function unless the ``FloodFillFlags/FLOODFILL_MASK_ONLY`` flag is set in the second variant of the function. See
 * the details below.
 ...
 * - SeeAlso *Ref:* imgproc_color_conversions
 */
 ...
 *
 *     ``ColorConversionCodes/COLOR_BayerBG2BGR`` , ``ColorConversionCodes/COLOR_BayerGB2BGR`` , ``ColorConversionCodes/COLOR_BayerRG2BGR`` , ``ColorConversionCodes/COLOR_BayerGR2BGR``
 *
 *     ``ColorConversionCodes/COLOR_BayerBG2GRAY`` , ``ColorConversionCodes/COLOR_BayerGB2GRAY`` , ``ColorConversionCodes/COLOR_BayerRG2GRAY`` , ``ColorConversionCodes/COLOR_BayerGR2GRAY``
 *
 * -   Demosaicing using Variable Number of Gradients.
 *
 *     ``ColorConversionCodes/COLOR_BayerBG2BGR_VNG`` , ``ColorConversionCodes/COLOR_BayerGB2BGR_VNG`` , ``ColorConversionCodes/COLOR_BayerRG2BGR_VNG`` , ``ColorConversionCodes/COLOR_BayerGR2BGR_VNG``
 *
 * -   Edge-Aware Demosaicing.
 *
 *     ``ColorConversionCodes/COLOR_BayerBG2BGR_EA`` , ``ColorConversionCodes/COLOR_BayerGB2BGR_EA`` , ``ColorConversionCodes/COLOR_BayerRG2BGR_EA`` , ``ColorConversionCodes/COLOR_BayerGR2BGR_EA``
 *
 * -   Demosaicing with alpha channel
 *
 *     ``ColorConversionCodes/COLOR_BayerBG2BGRA`` , ``ColorConversionCodes/COLOR_BayerGB2BGRA`` , ``ColorConversionCodes/COLOR_BayerRG2BGRA`` , ``ColorConversionCodes/COLOR_BayerGR2BGRA``
 *
 * Note:* The source image (src) must be of an appropriate type for the desired color conversion. see ColorConversionCodes
 * - SeeAlso ``cvtColor:dst:code:dstCn:hint:``
 */
 ...
 * ``TemplateMatchModes/TM_SQDIFF`` was used) or maximums (when ``TemplateMatchModes/TM_CCORR`` or ``TemplateMatchModes/TM_CCOEFF`` was used) using the
 * ``Core/minMaxLoc`` function. In case of a color image, template summation in the numerator and each sum in
 ...
 * parallel framework is enabled and if the rows of the image are at least twice the number returned by ``Core/getNumberOfCPUs``.
 *
 ...
 *
 * Note:* Since OpenCV 4.14, when mode is ``RetrievalModes/RETR_LIST`` and no hierarchy is requested, this function
 ...
 * pixels remain 0's, so the image is treated as binary . You can use ``Core/compare``, ``Core/inRange``, ``Imgproc/threshold`` ,
 * ``Imgproc/adaptiveThreshold``, ``Imgproc/Canny``, and others to create a binary image out of a grayscale or color one.
 * If mode equals to ``RetrievalModes/RETR_CCOMP`` or ``RetrievalModes/RETR_FLOODFILL``, the input can also be a 32-bit integer image of labels (CV_32SC1).
 ...
 * ``Imgproc/drawContours`` or ``Imgproc/fillPoly`` , can be different. Also, the function will most certainly give a wrong
 ...
 * 2D point set is required. The complexity of the ``Imgproc/convexHull`` function is `$$O(n log(n))$$` which is higher
 ...
 * of the 2D point set is required. The complexity of the ``Imgproc/convexHull`` function is `$$O(n log(n))$$` which
 ...
 * Compares two shapes.
 *
 * The function compares two shapes. All three implemented methods use the Hu invariants (see ``Imgproc/HuMoments``)
 *
 ...
 *  If the fit is found to be a parabolic or hyperbolic function then the standard ``Imgproc/fitEllipse`` method is used.
 ...
 *
 * The function cv::arrowedLine draws an arrow between pt1 and pt2 points in the image. See also ``Imgproc/line``.
 *
 * @param img Image.
 ...
 * @param thickness Thickness of lines that make up the rectangle. Negative values, like ``LineTypes/FILLED``,
 ...
 * @param thickness Thickness of the circle outline, if positive. Negative values, like ``LineTypes/FILLED``,
 ...
 * ``Imgproc/ellipse2Poly`` and then render it with ``Imgproc/polylines`` or fill it with ``Imgproc/fillPoly``. If you use the first
 * variant of the function and want to draw the whole ellipse, not an arc, pass `startAngle=0` and
 * `endAngle=360`. If `startAngle` is greater than `endAngle`, they are swapped. The figure below explains
 * the meaning of the parameters to draw the blue arc.
 *
 * ![Parameters of Elliptic Arc](ellipse)
 *
 * @param img Image.
 ...
 * function ``Imgproc/fillPoly`` . It can fill not only convex polygons but any monotonic polygon without
 ...
 * elliptic arc. It is used by ``Imgproc/ellipse``. If `arcStart` is greater than `arcEnd`, they are swapped.
 *
 * @param center Center of the arc.
 * @param axes Half of the size of the ellipse main axes. See ``Imgproc/ellipse`` for details.
 * @param angle Rotation angle of the ellipse in degrees. See ``Imgproc/ellipse`` for details.
 ...
 * using the specified font are replaced by question marks. See ``Imgproc/getTextSize`` for a text rendering code
 * example.
 *
 ...
 * @param fontScale Font scale factor that is multiplied by the font-specific base size.
 * @param thickness Thickness of lines used to render the text. See ``Imgproc/putText`` for details.
 * @param baseLine y-coordinate of the baseline relative to the bottom-most text
 * point.
 * @return The size of a box that contains the specified text.
 *
 * - SeeAlso ``putText:text:org:fontFace:fontScale:color:thickness:lineType:bottomLeftOrigin:``
 */
 ...
 *
 * - SeeAlso ``HoughLines:lines:rho:theta:threshold:srn:stn:min_theta:max_theta:use_edgeval:``
 */
 ...
 * *Note:* This function is for bindings use only. Use original function in C++ code
 *
 * - SeeAlso ``HoughCircles:circles:method:dp:minDist:param1:param2:minRadius:maxRadius:``
 */
